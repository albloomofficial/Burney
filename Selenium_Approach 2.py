...
def initialize_driver(page, driver_name="Chrome"):
    ...
    search_field.click()
    search_field.send_keys('slave_trade')
...
if __name__ == "__main__":
    ...
    with open('Srcs_Burney_{}_slave.csv'.format(str(filename_date)), 'w') as f:
            writer = csv.writer(f)
            writer.writerows(['origin', 'location', 'date', 'source', 'image_link', 'gale_number'])
    ...
    for i in range(int(multiprocessing.cpu_count()*3/4)):
        ...
        new_process = multiprocessing.Process(name=driver_names[i], target=run_thru_pages, args = (url_range, increment))
        new_process.start()
    pool.close()
    pool.join()
